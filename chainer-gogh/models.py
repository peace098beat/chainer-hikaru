...
class NIN:
    ...
    def forward(self, x):
        ...
        x1 = F.relu(
            self.model.conv2(F.average_pooling_2d(F.relu(y1), 3, stride=2)))
        y2 = self.model.cccp4(F.relu(self.model.cccp3(x1)))
        x2 = F.relu(
            self.model.conv3(F.average_pooling_2d(F.relu(y2), 3, stride=2)))
        y3 = self.model.cccp6(F.relu(self.model.cccp5(x2)))
        x3 = F.relu(getattr(self.model, "conv4-1024")
                    (F.dropout(F.average_pooling_2d(F.relu(y3), 3, stride=2), train=False)))
        return [y0, x1, x2, x3]
...
class VGG:
    ...
    def forward(self, x):
        ...
        y3 = self.model.conv3_3(
            F.relu(self.model.conv3_2(F.relu(self.model.conv3_1(x2)))))
        x3 = F.average_pooling_2d(F.relu(y3), 2, stride=2)
        y4 = self.model.conv4_3(
            F.relu(self.model.conv4_2(F.relu(self.model.conv4_1(x3)))))
...
#        self.pool_func = F.max_pooling_2d
    ...
    def forward(self, x):
        ...
        y6 = self.model.conv6_4(F.relu(F.dropout(self.model.conv6_3(
            F.relu(self.model.conv6_2(F.relu(self.model.conv6_1(x5))))), train=False)))
